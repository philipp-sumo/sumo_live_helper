...
function codeloop() {
    liveHelper = timer.setInterval(function(){
        getLoginstate();
        timer.setTimeout(fetchQuestion, 9000);
        timer.setTimeout(fetchInvolved, 37000);
    }, PollingTime());
}
    ...
    var questionRequest = Request({
        ...
        var latestsumoquestionid = response.json.results[0].id;
            ...
            if (ss.storage.lastpage == latestsumoquestionid) {
                ...
                if (alertexisting !== 1 && (prefs.watch_en === true && sumoquestionlocale === "en-US" || prefs.watch_es === true && sumoquestionlocale === "es" || prefs.watch_pt === true && sumoquestionlocale === "pt-BR"))  {
                    if (prefs.watch_firefox === true && sumoquestionproduct === "firefox" || prefs.watch_mobile === true && sumoquestionproduct === "mobile" || prefs.watch_firefoxos === true && sumoquestionproduct === "firefox-os" || prefs.watch_thunderbird === true && sumoquestionproduct === "thunderbird" || prefs.watch_webmaker === true && sumoquestionproduct === "webmaker"){
                ...
                var getUserdetails = Request({
                    ...
                    onComplete: function(userresponse) {
                        ...
                        if (userresponse.json.solution_count === 0){
                            ...
                            labeltext = ss.storage.solution_count + " solutions (" + (ss.storage.solution_count/ss.storage.answer_count*100).toFixed(1) + "%)\n" + ss.storage.helpfulness +  " helpful votes (" + (ss.storage.helpfulness/ss.storage.answer_count*100).toFixed(1) + "%)";}
    ...
    var involvedRequest = Request({
        ...
        var latestsumoinvolvedupdate = Date.parse(involved.json.results[0].updated);
            ...
            if (ss.storage.lastinvolvedtime == latestsumoinvolvedupdate) {
                ...
                if (ss.storage.lastinvolvedtime < latestsumoinvolvedupdate) {
                    timer.setTimeout(fetchInvolved, 3000);
                }
            }  
        }
    });
...
function classifyQuestion(sumoquestiontitle, sumoquestioncontent) {
    if (RegExp("(season|episode|vashikaran|gold medal|astrolog|black magic|husband wife|garcinia|reddit.com|fuck)[\\W]?\\d", "i").test(sumoquestiontitle, sumoquestioncontent) === true || sumoquestiontitle.length < 9 || sumoquestioncontent.length < 9 || getCharacterCount(sumoquestiontitle) < 6) {
